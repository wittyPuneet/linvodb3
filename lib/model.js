...
var LEVELUP_RETR_CONCURRENCY = 100; // We'll use that on a bagpipe instance regulating findById
...
function Model (name, schema, options) {
  ...
  schemas.indexes(self.schema).forEach(function(idx) { self.ensureIndex(idx) }); //Pass normalized schema instead of raw schema to build index.
 ...
 * @param {Object} query MongoDB-style query
 */
Model.prototype.find = function (query, projection, callback, quiet) {
...
Model.prototype.save = function (docs, cb, quiet) {
  ...
  var existingDocs = { };
  var stream = Cursor.getMatchesStream(this, { _id: { $in: _.chain(docs).map("_id").compact().value() } });
